...
export default function CompanyDetail() {
  ...
  function handleDownloadReport() {
    const lines = [
      'RASSAY AI — XAI CHURN RISK REPORT',
      `Generated: ${new Date().toLocaleString()}`,
      '',
      `COMPANY: ${customer.company_name || id}`,
      `Company ID: ${id}`,
      `Plan: ${customer.plan_type || 'N/A'}`,
      `Account Owner: ${customer.account_owner || 'Unassigned'}`,
      `Risk Level: ${customer.risk_level || 'N/A'}`,
      `AI Risk Score: ${(customer.risk_score * 100).toFixed(1)}%`,
      '',
      'XAI RISK FACTORS:',
      ...xaiFactors.map(
        (f, i) => `  ${i + 1}. ${f.feature_name} — Impact: ${f.impact_value.toFixed(4)} (${f.direction || 'N/A'})`,
      ),
      '',
      'Generated by Rassay AI Platform. For internal use only.',
    ]
    const blob = new Blob([lines.join('\n')], { type: 'text/plain' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `rassay-xai-report-${id}-${new Date().toISOString().split('T')[0]}.txt`
    a.click()
          ...
          <section className="grid gap-6 xl:grid-cols-[1fr_360px]">
            ...
            <aside className="space-y-4">
               ...
               <div className="grid gap-3">
                  ...
                  <ActionButton icon={Download} label="Download XAI Report (PDF)" onClick={handleDownloadReport} />
